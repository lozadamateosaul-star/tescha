...
const Alumnos = () => {
  ...
  const validateField = (name, value) => {
    ...
    switch (name) {
      case 'matricula':
        if (!value) error = 'La matrícula es obligatoria';
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2 flex items-center gap-2">
                    Matrícula *
                    <TooltipIcon text="Ingresa la matrícula del alumno (9-10 dígitos)" />
                  </label>
                  <div className="relative">
                    <input
                      type="text"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2 flex items-center gap-2">
                    Nivel Actual *
                    <TooltipIcon text="Nivel de inglés: A1 es principiante, C2 es avanzado" />
                  </label>
                  {formData.es_nuevo_ingreso === 'true' || formData.es_nuevo_ingreso === true ? (
                    <div>
                      <input
                        ...
                        readOnly
                      />
                      <p className="text-xs text-green-600 mt-1">✓ Nuevo ingreso siempre inicia en A1</p>
                    </div>
                  ) : (
                    <select
                      className="input"
